"""
TIMELAB · Vision Layer (Google Gemini Flash-Lite)
─────────────────────────────────────────────────
Shared photo-validation module used by every scanner that wants a final
visual sanity check on its top candidates.

Architecture:
- Called from each scanner AFTER all text/economic filtering, on the top-N
  candidates only (typically 5-10 per scanner). Minimal API cost.
- Single API call per candidate (one image, ~600 token output).
- Returns a VisionVerdict that callers use to BLOCK, FLAG, or PASS the alert.
- On any error, missing API key, or missing photo URL, returns "skip" verdict
  so the caller can fall through to existing behavior. Vision NEVER breaks
  the scanner — only enriches it.
- Total daily cost across all scanners: pennies/month at current volume.
...
it now governs all scanners (eBay, CC, Vinted) uniformly.
"""
...
def _build_prompt(brand_hint: str, target_id: str, model_hint: str, title: str, source: str = "") -> str:
    """Build the analysis prompt sent to Gemini alongside the image."""
    src_label = source or "second-hand"
    return f"""You are a watch arbitrage analyst reviewing a {src_label} listing photo.
...
def analyze_listing_photo(
    ...
    """
    Run Vision analysis on a listing's primary photo.

    `source` is a free-form label like "Vinted", "eBay", "CashConverters" used
    only in the prompt context. Has no bearing on logic; it just helps Gemini
    calibrate expectations (e.g., parts-only listings are common on Vinted but
    not on CashConverters).
    ...
    prompt = _build_prompt(brand_hint, target_id, model_hint, title, source)
