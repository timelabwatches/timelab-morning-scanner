...
def extract_brand(title: str) -> Optional[str]:
    ...
    for b in sorted(REPUTABLE_BRANDS, key=lambda x: -len(x)):
        cb = canon(b)
        # Word-boundary: prevents "oris" matching "motorista", etc.
...
# ─────────────────────────────────────────────────────────────────────────────
# LEVEL 1: Movement detection from listing description text
# No external API — pure regex/keyword scan.
# ─────────────────────────────────────────────────────────────────────────────
...
def target_requires_mechanical(target: Dict[str, Any]) -> bool:
    ...
    return any(w in family or w in tid or w in notes for w in AUTO_TARGET_SIGNALS)


def apply_description_filter(title: str, description: str, target: Dict[str, Any],
                              match: int, hits: int, targets: List[Dict[str, Any]]):
    """Level 1: reject quartz watches matched to automatic targets."""
    movement = detect_movement_from_text(title, description)
...
def run() -> None:
    ...
    for listing in all_listings:
        ...
        # Match on full_text (title + description)
    ...
    # if identity.get("target_id"): ...
        ...
        if target is not None:
            target, match, hits, detected_movement = apply_description_filter(
                listing.title, listing.description or "", target, match, hits, targets
            )
            if match < WP_MIN_MATCH:
                diag["dropped"]["no_brand"] += 1
                continue
    ...
    from datetime import datetime as _dt_wp
    hora_wp = _dt_wp.now().strftime("%H:%M")
    sep_wp = "━" * 26

    if not top:
        telegram_send(
            f"⌚ TIMELAB · Wallapop · {hora_wp}\n{sep_wp}\n"
            f"😴 Sin oportunidades hoy\n"
            f"Recolectados: {diag['collected']} | Escaneados: {diag['scanned']}"
        )
    else:
        lines = [f"⌚ TIMELAB · Wallapop · {hora_wp}", sep_wp, ""]
        for i, it in enumerate(top, 1):
            ...
            # ── Clean title ───────────────────────────────────────────────
            import re as _re_wp
            clean_title = li.title.strip().title()
            ...
            # ── Card ─────────────────────────────────────────────────────────
            lines.append("──────────────────────────")
            lines.append(f"{i}) {emoji} {bucket}  ·  Wallapop")
            lines.append(f"🏷 *{clean_title}*")
            lines.append("")
            lines.append(f"💶 Compra: *{li.price_eur:.0f}€*")
            ...
            lines.append("")
            lines.append(f"{mov_label}  ·  {li.cond or 'estado desconocido'}")
            if extras_str:
                lines.append(extras_str)
            lines.append("")
