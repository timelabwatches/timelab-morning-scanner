...
“”“Net profit after Catawiki commission + VAT on commission.”””
commission = close * CATWIKI_COMMISSION_RATE
commission_vat = commission * CATWIKI_COMMISSION_VAT
# Wallapop: no shipping arbitrage since buyer picks up or pays shipping directly
# But Timelab still ships for Catawiki so we keep the 35€ arb
SHIP_ARB = 35.0
...
if not top:
    ...
    for i, it in enumerate(top, 1):
        ...
        lines.append(f"   💶 Compra: {li.price_eur:.0f}€ | 🎯 Cierre est.: {it['close_est']:.0f}€")
        lines.append(
            f"   ✅ Neto est.: {it['net']:.0f}€ | ROI: {it['roi']*100:.1f}% | "
            f"Score: {it['score']} | Match: {it['match']} | KW: {it['kw_hits']}"
        )
        lines.append(f"   💰 Precio máx. sugerido: {it['max_buy_suggested']:.0f}€")
        lines.append(f"   📦 Estado: {li.cond} | 📸 Fotos: {li.images} | 📍 {li.location}")
        lines.append(f"   🏷️ Flags: {it['flags']} | Risk: {','.join(it['reason_flags']) or 'none'}")
        lines.append(f"   🎯 Confidence: {it['confidence']}")
        lines.append(f"   🧠 Why: {it['explain'].get('bucket_reason', '')}")
        lines.append(f"   🧩 Target: {it['target'].get('id', 'N/A')}")
