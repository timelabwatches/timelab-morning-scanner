...
Follows the same pattern as cashconverters_scanner.py.
...
WP_MIN_NET_EUR    (default 20)   min net profit €
...
from timelab_core.watch_id_engine import build_target_index, identify_watch
...
# Wallapop has an undocumented but stable JSON search endpoint.

# It returns structured data — no HTML scraping needed.

# ─────────────────────────────────────────────

WALLAPOP_SEARCH_URL = "https://api.wallapop.com/api/v3/general/search"

# Category 14000 = Relojes y joyas → subcategory watches

WALLAPOP_WATCH_CATEGORY = "14000"

# Search queries to cover your target brands
...
LADIES_TERMS = [
"señora", "senora", "mujer", "lady", "donna", "femme", "damenuhr", "chica",
]

SMARTWATCH_TERMS = [
"smartwatch", "connected", "apple watch", "watch series",
"galaxy watch", "wear os", "garmin", "fitbit",
]
...
Returns a list of WallapopListing objects.
"""
sess = session or requests.Session()
results: List[WallapopListing] = []
start = 0
page_size = 40  # Wallapop returns up to 40 per request
...
while len(results) < max_items:
    ...
    # Fallback: sometimes it's data.items directly
...
# TARGET LOADING (reuses cashconverters_scanner pattern)
...
raise ValueError("target_list.json inválido")
valid = [
t for t in targets
...
# MATCHING / SCORING (same logic as cashconverters_scanner)
...
"""Net profit after Catawiki commission + VAT. Shipping arbitrage +35€ (charged ~50€ to buyer)."""
...
"""Alternative estimate if sold on Chrono24 (6.5% fee, BIN price ~20% above Catawiki p50)."""
...
# BUG-2 FIX: Build 5-layer watch_id_engine index at startup.
try:
...
for listing in all_listings:
    ...
    # BUG-2 FIX: Augment with 5-layer watch_id_engine (brand+ref+family+target+confidence).
    # Fires when engine confidence is high or medium — overrides keyword matching.
    if target_index is not None:
        try:
            id_v2 = identify_watch(full_text, target_index)
            if id_v2.get("confidence_band") in ("high", "medium"):
                forced_v2 = next(
                    (t for t in targets if str(t.get("id", "")).upper() == str(id_v2.get("target_id", "")).upper()),
                    None,
                )
                if forced_v2 is not None:
                    target = forced_v2
                    match  = max(match, int(id_v2.get("confidence", 0)))
                    hits   = max(hits, 1)
                identity["match_confidence_band"] = id_v2.get("confidence_band", identity.get("match_confidence_band", "low"))
                identity["final_match_score"]     = max(int(identity.get("final_match_score", 0)), int(id_v2.get("confidence", 0)))
        except Exception:
            pass  # Engine failure is non-fatal
    ...
    # Wallapop precio máximo de compra sugerido
...
if not top:
    ...
    for i, it in enumerate(top, 1):
        ...
        # BUG-6 FIX: compact human-readable flags instead of raw dict
        flag_parts = []
        ...
        lines.append(f"{i}) {emoji} [{bucket}] [Wallapop] {li.title}")
        lines.append(f"   💶 Compra: {li.price_eur:.0f}€  |  🎯 Cierre Catawiki: {it['close_est']:.0f}€")
        lines.append(f"   ✅ Neto: {it['net']:.0f}€  |  ROI: {it['roi']*100:.1f}%  |  Score: {it['score']}")
        # BUG-3 FIX: Chrono24 alternative when relevant
        if c24.get("suggested") and c24.get("net", 0) > 0:
            lines.append(
                f"   📈 Chrono24 mejor → cierre {c24['close_est']:.0f}€  |  neto {c24['net']:.0f}€  |  ROI {c24['roi']*100:.1f}%"
            )
        elif c24.get("close_est", 0) > 0:
            lines.append(
                f"   📊 Chrono24 alt. → cierre {c24['close_est']:.0f}€  |  neto {c24['net']:.0f}€  |  ROI {c24['roi']*100:.1f}%"
            )
        lines.append(f"   💰 Máx. compra sugerido: {it['max_buy_suggested']:.0f}€  |  Match: {it['match']}  |  KW: {it['kw_hits']}")
        lines.append(f"   📦 Estado: {li.cond}  |  {flags_str}  |  📸 {li.images} fotos  |  📍 {li.location}")
        lines.append(f"   🏷️ Target: {it['target'].get('id', 'N/A')}  |  Risk: {','.join(it['reason_flags']) or 'ok'}")
...
if WP_DEBUG:
    dbg = [
        ...
        f"Thresholds: match>={WP_MIN_MATCH} | net>={WP_MIN_NET_EUR}€ | roi>={WP_MIN_NET_ROI} | haircut={WP_CLOSE_HAIRCUT}",
    ]
    telegram_send("\n".join(dbg))
```
