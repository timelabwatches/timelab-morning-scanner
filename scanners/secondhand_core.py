...
def fetch(url: str, session: requests.Session, timeout: int = 30, retries: int = 2) -> Optional[requests.Response]:
    """Fetch with retry. Some servers (RealCash) are slow on first connect from GitHub.
    Total worst-case wait: timeout * (retries+1) + sum(backoffs).
    """
    last_err = None
    for attempt in range(retries + 1):
        try:
            ...
            if r.status_code in (403, 429, 503):
                # rate-limit / blocked — retry with backoff
                log.warning(f"HTTP {r.status_code} on {url} (attempt {attempt+1}/{retries+1})")
            ...
            last_err = str(e)[:100]
            log.warning(f"Fetch timeout/conn error on {url} (attempt {attempt+1}/{retries+1}): {last_err}")
            if attempt < retries:
                time.sleep(3 + attempt * 2)
            continue
            ...
            return None
    log.warning(f"Failed after {retries+1} attempts: {url} ({last_err})")
    return None
...
def best_target(title: str, targets: List[Dict[str, Any]], raw_text: str = "") -> Tuple[Optional[Dict[str, Any]], int, int]:
    ...
    for trg in targets:
        ...
        is_gen = str(trg.get("id", "")).upper().endswith("_GENERIC")
        ...
        # Tiebreaker: specific target wins over _GENERIC at equal score
