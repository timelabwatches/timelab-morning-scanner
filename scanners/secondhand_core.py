...
class SiteConfig:
    ...
    paginate: bool = True                           # follow ?p=N paging
...
def best_target(title: str, targets: List[Dict[str, Any]]) -> Tuple[Optional[Dict[str, Any]], int, int]:
    ...
    for trg in targets:
        if violates_must_exclude(title, trg): continue
        ...
        if isinstance(kws, list) and len(kws) > 0 and not is_gen and hits < 1: continue
...
def run_scan(cfg: SiteConfig, targets_path: str = "target_list.json",
    ...
    for url in urls:
        ...
        target, score, hits = best_target(listing.title, targets)
        if not target:
