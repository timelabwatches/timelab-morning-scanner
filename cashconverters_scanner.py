...
def run() -> None:
    ...
    for u in listing_urls:
        ...
        cc_match_band = identity.get("match_confidence_band") or "medium"
        cc_ambiguity  = bool(identity.get("model_ambiguity", False))  # default False, not True
        gate = gate_decision(cc_match_band, sample_size, valuation_conf, net, roi, risk_allowed(target), cc_ambiguity)
        if gate == "SKIP":
            continue
        if gate == "REVIEW":
            bucket = "REVIEW"
