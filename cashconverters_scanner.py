...
def run() -> None:
    ...
    # BUG-2 FIX: Build 5-layer identification index from target_list at startup.
    # identify_watch() is far more accurate than keyword matching alone for:
    # vague titles ("Tissot automatic vintage"), partial refs, and ambiguous family names.
    try:
        target_index = build_target_index(targets)
    except Exception:
        target_index = None  # Fallback to keyword matching if engine fails
    ...
    for u in listing_urls:
        ...
        # Overrides keyword match when engine confidence is high or medium.
        if target_index is not None:
            try:
                id_v2 = identify_watch(listing.title, target_index)
                ...
                pass  # Engine failure is non-fatal — fall through to keyword matching
